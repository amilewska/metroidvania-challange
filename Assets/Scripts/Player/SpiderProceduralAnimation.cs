...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        defaultLegPosition = new Vector2[nbLegs];
        lastLegPosition = new Vector2[nbLegs];
    ...
    private void FixedUpdate()
    {
        ...
        Vector2[] desiredPosition = new Vector2[nbLegs];
        int indexToMove = -1;
        float maxDistance = stepSize;
        for (int i = 0; i < nbLegs; i++)
        {
            desiredPosition[i] = transform.TransformPoint(defaultLegPosition[i]);

            float distance = Vector3.ProjectOnPlane(desiredPosition[i] + velocity * velocityMultiplier - lastLegPosition[i], transform.up).magnitude;
        ...
        {
            Vector2 targetPoint = desiredPosition[indexToMove] + Mathf.Clamp(velocity.magnitude * velocityMultiplier, 0.0f, 1.5f) * (desiredPosition[indexToMove] -
            ...
            Vector2[] positionAndNormalizedBwd = MatchToSurfaceFromAbove(targetPoint+velocity*velocityMultiplier, raycastRange*(1f+velocity.magnitude),
                ((Vector2)transform.parent.up+velocity*10).normalized);
            ...
            {
                StartCoroutine(PerformStep(indexToMove, positionAndNormalizedBwd[0]));
            }
            else
            {
    ...
    private void OnDrawGizmosSelected()
    {
        for (int i = 0; i < nbLegs; i++)
        {
            ...
            Gizmos.DrawWireSphere(transform.TransformPoint(defaultLegPosition[i]), stepSize);
        }
    }
